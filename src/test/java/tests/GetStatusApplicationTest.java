...
public class GetStatusApplicationTest extends BaseTest {
  ...
  public void getStatusApplication() throws SQLException {
    ...
    assertAll(
        ...
        () ->
            assertEquals(
                response.getData().getKindofapplication(), "Получение свидетельства о браке"),
        () ->
            assertTrue(
                List.of("under consideration", "approved", "rejected")
                    .contains(response.getData().getStatusofapplication())),
        () ->
            assertTrue(
                response
                    .getData()
    ...
    String selectQuery =
        "SELECT applicationid, statusofapplication FROM reg_office.applications WHERE applicationid = ?";
